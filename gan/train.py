...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250,
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            noise_1 = sample_noise(batch_size, noise_size)
            noise_1 = noise_1.reshape((batch_size, noise_size, 1, 1)).to(device)
            fake_images = G(noise_1).to(device)
            d_error = discriminator_loss(D(real_images), D(fake_images))
            d_error.backward()
            D_solver.step()

            # update generator
            noise_2 = sample_noise(batch_size, noise_size)
            noise_2 = noise_2.reshape((batch_size, noise_size, 1, 1)).to(device)
            fake_images = G(noise_2).to(device)
