...
class SpectralNorm(nn.Module):
    ...
    def _update_u_v(self):
        ...
        for i in range(self.power_iterations):
            v_new = torch.matmul(torch.t(w.reshape(N, -1).data), u.data)
            v.data = l2normalize(v_new)
            u_new = torch.matmul(w.reshape(N, -1).data, v.data)
            u.data = l2normalize(u_new)
        w_new = w / torch.dot(u, torch.matmul(w.reshape(N, -1), v)).expand_as(w)
        setattr(self.module, self.name, w_new)
